...
const Question = () => {
    ...
    const answerBtn = answer.map((el, idx) =>el.length >4 ?<Abutton value={el} idx={idx} key={`button ${idx}`}>{el}</Abutton>:<Link to ='/Result'><Abutton value={el} idx={idx} key={`button ${idx}`}>{el}</Abutton></Link>)
